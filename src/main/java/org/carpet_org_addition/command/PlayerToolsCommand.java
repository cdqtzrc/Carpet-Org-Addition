...
public class PlayerToolsCommand {
                ...
                .then(CommandManager.argument("player", EntityArgumentType.player())
                        ...
                        .then(CommandManager.literal("action").then(CommandManager
                                        .literal("sorting").then(CommandManager.argument("item", ItemStackArgumentType.itemStack(commandBuildContext)).then(CommandManager.argument("this", Vec3ArgumentType.vec3())
                                                .then(CommandManager.argument("other", Vec3ArgumentType.vec3()).executes(context -> setAction(context, EntityArgumentType.getPlayer(context, "player"), FakePlayerActionType.SORTING))))))
                                .then(CommandManager.literal("clean").executes(context -> setAction(context, EntityArgumentType.getPlayer(context, "player"), FakePlayerActionType.CLEAN)))
                                .then(CommandManager.literal("fill").then(CommandManager.argument("item", ItemStackArgumentType.itemStack(commandBuildContext))
                                        .executes(context -> setAction(context, EntityArgumentType.getPlayer(context, "player"), FakePlayerActionType.FILL))))
                                .then(CommandManager.literal("stop").executes(context -> setAction(context, EntityArgumentType.getPlayer(context, "player"), FakePlayerActionType.STOP)))
                                .then(CommandManager.literal("craft")
                                        .then(CommandManager.literal("one").then(CommandManager.argument("item", ItemStackArgumentType.itemStack(commandBuildContext))
                                                .executes(context -> setAction(context, EntityArgumentType.getPlayer(context, "player"), FakePlayerActionType.CRAFT_ONE))))
                                        .then(CommandManager.literal("nine").then(CommandManager.argument("item", ItemStackArgumentType.itemStack(commandBuildContext))
                                                .executes(context -> setAction(context, EntityArgumentType.getPlayer(context, "player"), FakePlayerActionType.CRAFT_NINE))))
                                        .then(CommandManager.literal("four").then(CommandManager.argument("item", ItemStackArgumentType.itemStack(commandBuildContext))
                                                .executes(context -> setAction(context, EntityArgumentType.getPlayer(context, "player"), FakePlayerActionType.CRAFT_FOUR))))
                                ...
                                .then(CommandManager.literal("rename").then(CommandManager.argument("item", ItemStackArgumentType.itemStack(commandBuildContext))
                                        .then(CommandManager.argument("name", StringArgumentType.string()).executes(context -> setAction(context, EntityArgumentType.getPlayer(context, "player"), FakePlayerActionType.RENAME)))))
                                .then(CommandManager.literal("stonecutting").then(CommandManager.argument("item", ItemStackArgumentType.itemStack(commandBuildContext))
                                        .then(CommandManager.argument("button", IntegerArgumentType.integer(1)).executes(context -> setAction(context, EntityArgumentType.getPlayer(context, "player"), FakePlayerActionType.STONE_CUTTING)))))
                        )
                ));
    }
    ...
    private static int setAction(CommandContext<ServerCommandSource> context, ServerPlayerEntity fakePlayer,
        ...
        if (isFakePlayer(fakePlayer)) {
            ...
            } else if (action == FakePlayerActionType.CRAFT_2X2) {
    ...
    private static int getAction(CommandContext<ServerCommandSource> context, ServerPlayerEntity fakePlayer)
        ...
        if (isFakePlayer(fakePlayer)) {
            ...
            SendMessageUtils.sendTextMessage(player, TextUtils.appendAll(fakePlayer.getDisplayName(), ": ",
                    action.getActionText(context)));
        }
        return 1;
    }
    ...
    private static int openFakePlayerCraftGui(CommandContext<ServerCommandSource> context, ServerPlayerEntity fakePlayer)
        ...
        if (isFakePlayer(fakePlayer)) {
            ...
            SimpleNamedScreenHandlerFactory screen = new SimpleNamedScreenHandlerFactory((i, playerInventory, playerEntity)
                    ...
                    TextUtils.getTranslate("carpet.commands.playerTools.action.type.craft_gui"));
            player.openHandledScreen(screen);
        }
        return 1;
    }
