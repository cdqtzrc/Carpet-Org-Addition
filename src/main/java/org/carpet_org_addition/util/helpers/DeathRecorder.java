...
public class DeathRecorder {
    ...
    public void write() throws IOException {
        ...
        ModLevelFormat modLevelFormat = new ModLevelFormat(player.server, DEATH_LOG, this.playerName);
        // 死亡信息是在统计信息更新之前记录的，所以这里获取的死亡次数需要+1
        File file = modLevelFormat.createModFile((getDeathCount(this.player) + 1) + ".nbt");
        // 将NBT写入本地文件
    ...
    public static NbtCompound load(ServerPlayerEntity player, int number) throws IOException {
        ...
        ModLevelFormat modLevelFormat = new ModLevelFormat(player.server, DEATH_LOG, playerName);
        File file = modLevelFormat.createModFile(number + NBT);
