...
public class GameUtils {
    ...
    public static EntityPlayerMPFake createFakePlayer(String username, MinecraftServer server, Vec3d pos,
                                                      double yaw, double pitch, RegistryKey<World> dimensionId,
    ...
    private static EntityPlayerMPFake trySpawn(MinecraftServer server, Vec3d pos, float yaw, float pitch,
                                               RegistryKey<World> dimensionId, GameMode gamemode, boolean flying,
                                               GameProfile gameprofile, ServerWorld worldIn) {
        // 生成假玩家的逻辑似乎不在主线程
        ...
        server.getPlayerManager().onPlayerConnect(new FakeClientConnection(NetworkSide.SERVERBOUND), instance,
                new ConnectedClientData(gameprofile, 0, instance.getClientOptions()));
        ...
        server.getPlayerManager().sendToDimension(new EntitySetHeadYawS2CPacket(instance,
                (byte) ((int) (instance.headYaw * 256.0F / 360.0F))), dimensionId);
